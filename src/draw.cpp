...
nogrid:;
    ...
    ssglDrawEdges(&(SS.nakedEdges), true, hStyle( Style::DRAW_ERROR ));
    ...
    if(SS.centerOfMass.draw && !SS.centerOfMass.dirty) {
      ...
      const int subdiv = 16;
      ssglWriteText(ssprintf("(%.3f, %.3f, %.3f)", p.x, p.y, p.z),
      ...
      glBegin(GL_LINES);
      {
        ...
        ssglVertex3v(p.Minus(v));
        ssglVertex3v(p.Plus(v));
      }
      glEnd();
