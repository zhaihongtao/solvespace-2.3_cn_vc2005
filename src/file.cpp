...
bool SolveSpaceUI::LoadFromFile(const std::string &filename) {
    ...
    if(fileLoadError) {
        Error("无法识别文件中的数据，此文件可能已损坏，或来自程序的新版本。");
    ...
    while(fgets(line, (int)sizeof(line), fh)) {
        ...
        if(e) {
            *e = '\0';
            ...
            // so import it.
            if (SK.style.FindByIdNoOops(sv.s.h) == 0) {
                SK.style.Add(&(sv.s));
            }
            //sv.s = {};
			ZERO(&sv.s);
...
/*
    ...
    return std::equal(wa.begin(), wa.end(), wb.begin(), 
