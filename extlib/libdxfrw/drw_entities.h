...
public:
    ...
    ~DRW_LWPolyline() {
		int nCount = vertlist.size ();
		for ( int i = 0; i < nCount; i ++ )
		{
			DRW_Vertex2D * pItem = vertlist [ i ];
...
public:
    ...
    DRW_Spline(const DRW_Spline& p):DRW_Entity(p){
		...
		int nCount = p.knotslist.size();
		for ( int i = 0; i < nCount; i ++ ) knotslist.push_back ( p.knotslist [ i ] );
        //for(double v : p.knotslist) knotslist.push_back(v);

		nCount = p.weightlist.size();
		for ( int i = 0; i < nCount; i ++ ) weightlist.push_back ( p.weightlist [ i ] );
        //for(double v : p.weightlist) weightlist.push_back(v);

		nCount = p.controllist.size();
		for ( int i = 0; i < nCount; i ++ ) controllist.push_back ( p.controllist [ i ] );
        //for(DRW_Coord *v : p.controllist) controllist.push_back(new DRW_Coord(*v));

		nCount = p.fitlist.size();
		for ( int i = 0; i < nCount; i ++ ) fitlist.push_back ( p.fitlist [ i ] );
        //for(DRW_Coord *v : p.fitlist) fitlist.push_back(new DRW_Coord(*v));
    }
    
    ~DRW_Spline() {
		int nCount = controllist.size();
		for ( int i = 0; i < nCount; i ++ )
		{
			DRW_Coord * item = controllist [ i ];
			delete item;
		}
...
public:
    ...
    ~DRW_HatchLoop() {
		...
		int nCount = objlist.size();
		for ( int i = 0; i < nCount; i ++ )
		{
			DRW_Entity * item = objlist [ i ];
			delete item;
		}
    }
...
public:
    ...
    ~DRW_Hatch() {
		...
		int nCount = looplist.size();
		for ( int i = 0; i < nCount; i ++ )
		{
			DRW_HatchLoop * item = looplist [ i ];
			delete item;
		}
    }
...
public:
    ...
    ~DRW_Leader() {
		...
		int nCount = vertexlist.size();
		for ( int i = 0; i < nCount; i ++ )
		{
			DRW_Coord * item = vertexlist [ i ];
			delete item;
		}
    }
